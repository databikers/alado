...
type RouterUseArgs = [HttpMethod, string, Context<any>, (request: Request) => void] | [HttpMethod, string, (request: Request) => void];
export class Router {
  ...
  use(
    ...
    try {
      ...
      if (this.options.openApiDoc?.enable && this.options.openApiDoc?.route === clearRoute(uri) && method === HttpMethod.GET) {
    ...
    if (this.options.cors?.enable) {
      if (this.options.cors.allowedMethods.has(optionsDataKey)) {
        ...
        this.routes.set(optionsRouteRegEx, { route: optionsRouteRegEx, path: pathData, context, handler: (request: Request) => {
            return {
              statusCode: 204,
              headers:  {
